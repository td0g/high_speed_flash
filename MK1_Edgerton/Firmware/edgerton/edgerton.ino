...
0.2.5  2019-09-26
  Fixed issue where flash would only fire for 500 ns, regardless of menu option selection

0.2.6 2019-11-03
  New MAX VOLTAGE feature, unit will not flash if voltage exceeds value
  More intelligent capacitor voltage calibrating system (shows live output of capacitor voltage)
...
//Basic Configuration
  #define MIN_BATTERY_VOLTAGE 8.5   //Capacitors will not charge if battery voltage is below this voltage
  #define MAX_CAPACITOR_VOLTAGE 125 //Flash will not trigger above this voltage
  #define BUTTON_DEBOUNCE 150   //Millis for button debounce
...
//Pin Configuration
  ...
  #define SOUND_PIN 11
  #define FLASH_PIN 8       //TC4452 Gate Driver Input (via High-Pass)
...
//Calibration
  ...
  EEPROM.get(EEPROM_HISTORY_END + 4, capacitorVoltCal);
  initialSetup();
...
//Get flash count
  flashCount = readFlashCount();

//Turn off display colon
  display.colon(0);
}

void loop() {runDisplay();runUI();runSerial();runAudio();}
